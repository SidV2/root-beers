    ...
    actions$.pipe(
      ...
      switchMap(({ drinkId, offset, length }) =>
        drinksService.getReviews(drinkId, offset, length).pipe(
          map((response) => DrinksActions.loadReviewsSuccess({ reviews: response.items, total: response.total })),
...
export const loadMoreReviews$ = createEffect(
  (actions$ = inject(Actions), drinksService = inject(DrinksService)) =>
    actions$.pipe(
      ofType(DrinksActions.loadMoreReviews),
      switchMap(({ drinkId, offset, length }) =>
        drinksService.getReviews(drinkId, offset, length).pipe(
          map((response) => DrinksActions.loadMoreReviewsSuccess({ reviews: response.items, total: response.total })),
          catchError((error) => of(DrinksActions.loadReviewsFailure({ error: error.message }))),
        ),
      ),
    ),
